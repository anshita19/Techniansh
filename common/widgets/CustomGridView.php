 ...
 * @since 2.0
 */
class CustomGridView extends BaseListView {
     ...
     * @see \yii\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
     */
    public $tableOptions = ['class' => 'table table-bordered table-hover datatable-highlight dataTable no-footer'];
     ...
     * @see \yii\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
     */
    public $options = ['class' => 'grid-view', 'data-gridurl' => 'index'];
    ...
    public $summaryOptions = ['class' => 'dataTables_info', 'role' => 'status', 'aria-live' => 'polite'];
    public $summary = 'Showing {begin} to {end} of {totalCount} entries.';
    //public $pager = ['maxButtonCount' => 3]; // Set maximum number of page buttons that can be displayed
     ...
     */
    public $layout = "<div class='datatable-scroll'>{items}</div>\n
    ...
    public function init() {
        ...
        $this->defaultPageSize = isset($this->defaultPageSize) ? $this->defaultPageSize : \Yii::$app->params['defaultPageSize'];
        $this->dataProvider->pagination->pageSizeParam = isset($_REQUEST['per-page']) ? $_REQUEST['per-page'] : $this->defaultPageSize;
        $this->dataProvider->pagination->pageSize = (!isset($_REQUEST['per-page'])) ? $this->defaultPageSize : $_REQUEST['per-page'];
        
        $param = Yii::$app->getRequest()->getQueryParams();
        if(!empty($param)) $this->pjaxUrl = Url::to(array_merge([$this->defaultUrl],$param));
        else $this->pjaxUrl = Url::to([$this->defaultUrl]);

        $this->options['data-gridurl'] = $this->pjaxUrl;
