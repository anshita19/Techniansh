...
class MetadataComponent extends Component {
    ...
    public $defaultNamespace = '\\backend\controllers\\';
    ...
    public function getActions($controller, $module = null) {
        ...
        if ($module != null) {
            ...
            $path = Yii::$app->basePath . DIRECTORY_SEPARATOR . 'controllers';
        }
        if(file_exists($path . DIRECTORY_SEPARATOR . $controller . '.php')){
            include_once($path . DIRECTORY_SEPARATOR . $controller . '.php');
            $reflection = new \ReflectionClass($this->defaultNamespace . $controller);
            ...
            foreach ($methods as $method) {
                if (strpos($method->name, 'action') === 0 and ctype_upper($method->name[6])) {
                    $actions[$i] = str_replace('action', '', $method->name);
                    $i++;
                }
            }
            return $actions;
    ...
    public function getActions1($controller, $module = null) {
